...
function selectdata(xmlObject){
	...
	for(var i=0;i<rows.length;i++){
		...
		if(document.all)
			setCellLink("title"+i,"insert.jsp?actiontype=update&id="+document.getElementById("id"+i).innerText+""+""
					+"&"+url0+url+urlOrder+encodeURI(urlSearch)+urlPage+pager+"","");
		else
			setCellLink("title"+i,"insert.jsp?actiontype=update&id="+document.getElementById("id"+i).textContent+""+""
					+"&"+url0+url+urlOrder+encodeURI(urlSearch)+urlPage+pager+"","");
...
function sort(fieldName){
	selectDataLoad1();
	var fields=new Array("title","user_id","type","department","operate_state","open_state","score","academic","apply_state","create_user","create_time","audior_name","source","key_word","ucg");
	if(order==null){
		order="true";
...
function get_type(type){
	switch(type){
		...
		case 'department_type':
			return 'string';
		case 'operate_state_type':
			return 'string';
		case 'open_state_type':
			return 'string';
		case 'score_type':
			return 'string';
		case 'academic_type':
			return 'string';
		case 'apply_state_type':
			return 'string';
		case 'create_user_type':
			return 'string';
		case 'create_time_type':
			return 'string';
		case 'audior_name_type':
			return 'string';
		case 'source_type':
			return 'string';
		case 'key_word_type':
