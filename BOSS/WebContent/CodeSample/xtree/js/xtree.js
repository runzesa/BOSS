...
WebFXTreeAbstractNode.prototype.add = function (node, bNoIdent, iLast) {
	...
	if (root.rendered) {
		...
		//����Ϊ��ӣ�Ϊ���ж��Ƿ�Ϊ����xml������ݣ������Ƿ�Ϊ������Ϊĩ���ڵￄ1�7
		if (!iLast)
			this._last = true;
		var foo = this;
...
WebFXTree.prototype.getSelectIds = function() {
	var str = "";
	// ￄ1�7��ￄ1�7
	var inputName = webFXTreeConfig.inputName;
	try{
		var sels = document.getElementsByName( inputName );
...
// ��ȡѡ��˵����󣬰ￄ1�7(��text��value
WebFXTree.prototype.getSelectObj = function() {
	var obj = new Array( );
	// ￄ1�7��ￄ1�7
	var inputName = webFXTreeConfig.inputName;
	try{
		...
		for(var i = 0; i < size; i++ ){
			if( sels[ i ].checked ){
				var j = obj.length;
				obj[ j ] = new Object;
				obj[ j ].text = sels[ i ].text;
				obj[ j ].value = sels[ i ].value;
			}			
		}
...
// ���õ�ǰѡ��ѡ��ֵ
// winder 2006-8-11
WebFXTree.prototype.setSelectValue = function( value ) {
	var v = "," + value + ",";
	if ( document.readyState != "complete" || this.loaded == false )
	{
		var self = this;
		window.setTimeout( function(){ self.setSelectValue( value ); }, 1000);
		return;
	}
	try{
		// ￄ1�7��ￄ1�7
...
WebFXTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	// ￄ1�7��ￄ1�7
	var inputName = webFXTreeConfig.inputName;
	// �޸ģ�winder��2006-5-10����ӱ�ǩ���ￄ1�7
	if(this.radio=="Y"||this.radion=="y"||this.radio=="true"){
		var inputID = inputName + this.id;
	...
	// �޸ģ�winder��2006-5-10��������ѡ�Ŧ���幦��
	if(this.radio=="Y"||this.radion=="y"||this.radio=="true"){
		str+="<input type=\"Radio\" id=\""+inputID+"\" name=\""+inputName+"\" text=\"" + label + "\" value=\""+svalue+"\">"; //"+selectedstr+"
	}else if(this.checkbox=="Y"||this.checkbox=="y"||this.checkbox=="true"){
		if(this.checked=="Y"||this.checked=="y"||this.checked=="true"){		
			selectedstr="checked"	
		}
		str+="<input type=\"Checkbox\" id=\""+inputID+"\" name=\""+inputName+"\" text=\"" + label + "\" value=\""+svalue+"\" "+selectedstr+">";
	}
	
	str +=  "<label for=\""+inputID+"\">";
...
webFXTreeConfig.loadingText = "���ڼ���...";
webFXTreeConfig.loadErrorTextTemplate = "���ش��� \"%1%\"";
webFXTreeConfig.emptyErrorTextTemplate = "���� \"%1%\" û��TreeNode�ڵ㣡";
	...
	if( oXmlDoc == null || oXmlDoc.documentElement == null) {
		...
		for (var i = 0; i < l; i++) {
			...
			if (cs[i].tagName == "TreeNode") {
				...
				jsParentNode.add( _xmlTreeToJsTree(cs[i]), true, iLast);
			}
		}
