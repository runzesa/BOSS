...
dhtmlXVaultObject.prototype.create = function(htmlObject)
{
    ...
    var str = "<table style='background-color:#EDEEEF;border: 1px solid #7A7C80;' border='0'>" +
              ...
              "<img _onclick='UploadControl.prototype.uploadAllItems()' _ID='ImageButton3'  src='../images/upload/btn_upload.gif' style='cursor:pointer'/></td>" +
              "<td style='width: 140px; height: 32px;' align='right'>" +
              "<img _onclick='return UploadControl.prototype.removeAllItems()' _ID='ImageButton3'  src='../images/upload/btn_clean.gif' style='cursor:pointer;margin-right:20px'/></td></tr></table>" +
              "<div _id='fileContainer' style='width:84px;overflow:hidden;height:32px;left:0px;direction:rtl;position:absolute;top:211px'>" +
              "<img style='z-index:2' src='../images/upload/btn_add.gif'/>" +
...
dhtmlXVaultObject.prototype.addFile = function()
{
    ...
    cellList.innerHTML = "<a href='#remove' class='link' style=\"color:#ff0000;font-size:12px;\">删除</a>";
    //alert(currentId);
    ...
    cellList = document.createElement("td");
    cellList.style.cssText = ";width:140px;height:30px;";
    cellList.innerHTML = "<a href='#upload' class='link' style=\"color:#0000ff;font-size:12px;\">上传</a>";
    cellList.firstChild.onclick = function() {
...
{
    //-------------------------------------------
    var srcImgPic = "../images/upload/ico_image.png";
    var srcImgVideo = "../images/upload/ico_video.png";
    var srcImgSound = "../images/upload/ico_sound.png";
    var srcImgArchives = "../images/upload/ico_zip.png";
    var srcImgFile = "../images/upload/ico_file.png";
...
{
    var srcImgProgress = "../images/upload/pb_demoUload.gif";
    ...
    var xmlHttp = this.createXMLHttpRequest();
        ...
        if (xmlHttp.status == 200)
        {
            if (xmlHttp.responseText)
            {
                    ...
                    //if (parseInt(xmlHttp.responseText) != -1 || parseInt(xmlHttp.responseText) != -2)
                        ...
                        }else if (parseInt(xmlHttp.responseText) == -2)
                        {
                            ...
                            var tblContent = this.getCurrentTblContent(id);
                            tblContent.rows[1].cells[0].innerHTML += "<font class='text' style=\"font-size:12px;color:#ff0000;\">文件上传格式不确,上传失败</font>";
                            tblContent.rows[1].cells[0].vAlign = "top";
...
//TODO this.uploadForm.action = this.pathUploadHandler + "?sessionId=" + this.sessionId + "&fileName=" + this.getFileName(this.getCurrentInputFile(id).value)+"&userfile="+this.getCurrentInputFile(id).id;
